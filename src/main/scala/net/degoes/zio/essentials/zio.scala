...
object zio_values {
  ...
  /**
    * Using `ZIO.defer` method. wrap Scala's `println` method to import it
  ...
  /**
    * Using the `ZIO.defer` method, wrap Scala's `readLine` method to import it
    * into the world of pure functional programming.
    * and identify the correct ZIO type.
    */
  val getStrLn: ??? = readLine ?
...
object zio_failure {
  ...
  /**
    * Using `IO.defer`. Import a synchronous effect with a strange code
    */
  ...
  /**
    ...
    */
  val terminate3: UIO[Int] = IO.defer("Hello".toInt) ?

  case class DomainError()

  val veryBadIO: IO[DomainError, Int] =
    IO.defer(5 / 0).flatMap(_ => IO.fail(DomainError()))
...
object zio_effects {
  ...
  /**
    * Using `ZIO.defer` method. increment the given int and identify the correct ZIO type
    */
  def increment(i: Int): ??? = (i + 1) ?

  /**
    * Using `ZIO.sync` method. wrap Scala's `toInt` method to convert a given string value to int
  ...
  /**
    * Use `ZIO.async` method to implement the following `sleep`method
  ...
  /**
    * Wrap the following Java callback API, into an `IO` using `IO.async`
  ...
  /**
    * Using `asyncMaybe` wrap the following Java callback API into an `IO`
  ...
  /**
    * using `asyncInterrupt` wrap the following Java callback API into an `IO`
    * and use better ZIO type alias.
    */
  ...
  /**
    * using `ZIO.async` import a pure value of type int
...
object zio_interop {
  ...
  /**
    ...
    */
  val task1: Task[Int]       = IO.sync("wrong".toInt)
...
object zio_resources {
  ...
  class InputStream private (is: FileInputStream) {
    def read: IO[Exception, Option[Byte]] =
      IO.defer(is.read).map(i => if (i < 0) None else Some(i.toByte))
    def close: IO[Exception, Unit] =
      IO.defer(is.close())
  }
  object InputStream {
    def openFile(file: File): IO[Exception, InputStream] =
      IO.defer(new InputStream(new FileInputStream(file)))
  }
...
object zio_dependency_management {
  ...
  /**
    ...
    * - implement a trait `Live` that extends the module and implement all helper functions.
    */
  ...
  object Main extends Runtime[Configuration] {
    ...
    /**
      * using `provide` return an UIO that contains the port
      */
    val port: UIO[Int] = ???

    /**
      * Using `provide` return an UIO that print out in the console
